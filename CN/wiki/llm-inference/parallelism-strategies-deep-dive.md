...
> 3. **Decode 是内存带宽瓶颈**。Ring Attention "通信免费"前提是 $T_{\text{compute}} \geq T_{\text{comm}}$。Decode 每步计算极小（1 个 query token × full KV，全是内存读）。Meta 论文 Table 6 实测：Llama 3 405B / TP8 上 TTIT *随 CP 单调上升* —— CP1 46.26 ms → CP4 71.31 ms。
...
> [!question]+ Shiki —— TensorRT-LLM 的 CP 到底实现了什么？（2026-05-20）
>
> 翻了本地 TRT-LLM repo（`tensorrt_llm/mapping.py:25-34`）。源码里 `CpType` 枚举有 **4 个值**，全部通过 `cp_config={"cp_type": ...}` 暴露。默认 `ULYSSES`。三个是真实现，一个是占位。
>
> | `cp_type` | 状态 | 算法 | 代码位置 |
> | --------- | ---- | ---- | -------- |
...
> - **Ulysses** 在 **TP 通信组里** 做 CP（head 维 all-to-all 复用 TP 通信器）。
> - **Star** 在 **请求调度层** 做 CP（上下文切块发到 cp_size 张卡，每卡跑标准的局部 attention）。
> - **Helix** 在 **attention kernel 内部** 做 CP（token 切 KV + MNNVL alltoall）。
>
> 所以 "真 CP" 不是 Helix 专属标签 —— Ulysses 和 Star 也都是真 CP，只是被 relabel 了或者移到了更上层。
>
> 逐项细节：
>
> - **Ulysses（默认）**就是上一个 Q&A 提到的那种 head 维度 all-to-all，但 TRT-LLM 不是把它做成独立的并行轴，而是把 CP 组 *折叠进* TP 组。用途：单节点多卡的长 prefill。硬限制：`attn_cp_size != 1 && cp_type == ULYSSES` 直接报错（`mapping.py:109-112`）。
> - **Star Attention** 是 *稀疏* CP 路径 —— 不是通用长序列方案。每个 rank 只看到自己的 block + anchor block。针对 needle-in-a-haystack 类的超长上下文检索（例子脚本目标 NIAH 到 512K）。价值定位和 Ring/Ulysses 完全不同：拿精度换计算。
> - **Helix** 是 *decode 阶段* CP 路径 —— 就是 Meta 论文说 decode 不该用 CP 的那种事，但 Helix 用两招绕开 cost：(a) 用 MNNVL `all_to_all`（NVL72 级 fabric）替掉 NCCL 的 ring rotation，(b) FFN 把 CP rank 重新征用为 TP，CP 通信只在必须的地方付。这是 NVIDIA 对 "GB200/NVL72 时代该不该让 CP 进 decode" 的回答。
> - **Ring** 保留但没实现 —— 可能是给未来 pass-KV / pass-Q kernel 的位置，或者只是和训练侧的 Ring Attention 词汇对齐。今天 TRT-LLM 里所谓的 "ring 语义" 其实是通过 Ulysses 风格 head 切分实现的。
>
> **所以公开文档那句 "Context parallelism distributes the processing of long sequences across multiple GPUs" 实际上把三种完全不同的算法藏在同一个旋钮后面** —— Ulysses 走 vanilla 长 prefill、Star 走稀疏长上下文检索、Helix 走 MNNVL decode。**不是一个 CP，而是三个**。

> [!question]+ Shiki —— 为什么 TRT-LLM 有 4 个 CP 变体而 vLLM/SGLang 没有？Meta 的 inference CP 真的被用了吗？（2026-05-20）
>
> 两个相关问题，都关乎推理 CP 的 *部署现实*。短答案：**三家主流引擎都做了 CP，但 TRT-LLM 出 4 个变体，vLLM/SGLang 各出 1 个**；**Meta 论文的具体算法在 2026 实际上是单客户技术**。
>
> **2026-05 引擎矩阵：**
>
...
> | **vLLM** | PCP（PR #28718，2025-11-19） | DCP（PR #24864，KV 去重 trick） | **2**（PCP + DCP） |
> | **SGLang** | DSV4 的 NSA Prefill CP（`--enable-nsa-prefill-context-parallel`） | — | **1**（DSV4-scoped） |
>
> **为什么 TRT-LLM 出 4 个而 vLLM/SGLang 各出 1 个 —— 四个驱动因素：**
>
...
> | **TRT-LLM** | ❌ **没有**实现 Meta 的 pass-KV / pass-Q。出的是 Ulysses（head 维 alltoall）+ Star（anchor-block 稀疏）+ Helix（MNNVL decode）—— 同一个问题，完全不同的算法。|
> | **vLLM** | ✅ PCP 2025-11-19 合并（PR [#28718](https://github.com/vllm-project/vllm/pull/28718))—— 自己的算法（AllReduce + ReduceScatter，Ulysses-leaning），**不是** Meta 的 pass-KV / pass-Q。|
> | **SGLang** | ✅ DSV4 的 NSA Prefill CP（`--enable-nsa-prefill-context-parallel`，2026-05 前后多个 PR）—— round-robin token + `cp_all_gather`，**不是** Meta 算法。scope 限于一个模型。|
