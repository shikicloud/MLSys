...
**训练 vs 推理**：CP 历史上是 *训练* 技术 —— 激活和梯度内存随 $S$ 暴涨，最先压垮单卡。在 *推理* 里，KV cache 压缩（[[saw-int4|SAW-INT4]]、MLA、KV 裁剪）和 [[prefill-decode-disaggregation|PD 分离]] 通常是更早的选择，不一定先考虑 CP。**Meta** 是唯一公开演示了推理 CP 的前沿实验室：["Context Parallelism for Scalable Million-Token Inference" (arXiv:2411.01783)](https://arxiv.org/abs/2411.01783) 展示 Llama 3 405B 在 128 张 H100 上 77 秒完成 1M token prefill，用 **pass-KV / pass-Q 混合** ring attention。引擎层面今天的支持：**vLLM** 同时出了 *Decode Context Parallel*（DCP，PR [#24864](https://github.com/vllm-project/vllm/pull/24864), 2025-10，"仅 KV 去重"窄义变体）和 *Prefill Context Parallel*（PCP，PR [#28718](https://github.com/vllm-project/vllm/pull/28718), 2025-11-19 合并）—— RFC 路径虽然被关了，但实现走直接 PR 路径绕开。**TensorRT-LLM** 把 `context_parallel_size` 作为一等公民 API 暴露（[文档](https://nvidia.github.io/TensorRT-LLM/features/parallel-strategy.html)）。**SGLang** *也* 出了 prefill CP，但只针对 DeepSeek-V4 的 NSA attention：`--enable-nsa-prefill-context-parallel` flag（2026-05 前后多个 PR，如 [#23882](https://github.com/sgl-project/sglang/pull/23882)、[#24560](https://github.com/sgl-project/sglang/pull/24560)）。他们 [2026-01 的 "PP > CP" 立场](https://www.lmsys.org/blog/2026-01-15-chunked-pipeline/) 是针对 *通用* 百万 token serving；对 DSV4 NSA 这种特定 attention 变体，他们还是做了 CP。**其它前沿实验室（Google Gemini、Anthropic Claude、OpenAI GPT-4）没公开过自己的长上下文 serving 是否用 CP** —— 任何"它们用 CP"的说法都是未证实猜测。

> [!question]+ Shiki —— vLLM / SGLang / TensorRT-LLM 现在在推理时支持 CP 吗？为什么 CP 之前都只针对训练？（2026-05-20）
>
> 是的，部分支持，但分化明显。**引擎矩阵：**
>
> - **vLLM**：**同时**出了 *Decode Context Parallel*（DCP，`--decode-context-parallel-size N`）和 *Prefill Context Parallel*（PCP，PR [#28718](https://github.com/vllm-project/vllm/pull/28718)，2025-11-19 合并）。DCP 是窄义的 **KV cache 去重 trick** —— 针对 MLA 类模型 `tp_size > num_kv_heads` 时 KV 在 TP rank 间复制的浪费。PCP 才是真正的 prefill 阶段 sequence 维切分 —— 复用 DCP 的 KV cache interleaving 基础设施（`dcp_kv_cache_interleave_size` 被泛化成 `cp_kv_cache_interleave_size`），跑在现有 Triton attention backend 上，同时支持 MLA 和 GQA。值得注意：PCP 走直接 PR 路径合并，**绕开了**早期被关的 RFC（[#22693](https://github.com/vllm-project/vllm/issues/22693)、[#26133](https://github.com/vllm-project/vllm/issues/26133)）—— RFC 状态预测不了 shipping 状态。PR 没明说用什么算法（[[ring-attention|Ring]] / [[deepspeed-ulysses|Ulysses]] / pass-KV-pass-Q），但通信模式是 AllReduce + ReduceScatter（`cp_lse_ag_out_ar`），更像 Ulysses 风格的混合而非纯 Ring。
> - **SGLang**：混合状态。*通用* 百万 token serving 选了 **流水线并行**（[LMSYS 博客 2026-01-15](https://www.lmsys.org/blog/2026-01-15-chunked-pipeline/)：CP "需要侵入性的 per-model attention 修改"，PP 通信量更低 —— Qwen3-235B / H20 上 1M TTFT 420 秒）。通用 Decode-CP 的 feature request [#12196](https://github.com/sgl-project/sglang/issues/12196)（2025-10）已关闭/不活跃。**但**针对 DeepSeek-V4 的 NSA attention，他们做了 prefill CP：`--enable-nsa-prefill-context-parallel`（2026-05 前后多个 PR，包括 [#23882](https://github.com/sgl-project/sglang/pull/23882)、[#24560](https://github.com/sgl-project/sglang/pull/24560)）。算法：round-robin token 切分 + `cp_all_gather`（Ulysses-leaning）。测试配置：B300 TP=8 CP=8。立场可以概括为"不做通用 CP 层，但 attention 变体要求时做 per-model CP"。
> - **TensorRT-LLM**：`context_parallel_size` + `cp_config` 作为一等公民 API 暴露（[文档](https://nvidia.github.io/TensorRT-LLM/features/parallel-strategy.html)）。公开文档很薄，但源码（`tensorrt_llm/mapping.py:25-34`）实际给了 **4 个** `CpType` 变体：`ULYSSES`（默认）、`STAR`（Star Attention）、`HELIX`（Helix Parallelism）、`RING`（仅枚举占位，无实现）。逐项拆解见下面专门的 Q&A。
>
> **CP 为什么历史上只用于训练 —— 四个技术错配：**
>
> 1. **持久 KV cache**。训练时每次 forward 重算 KV，旋转随便。推理 decode 时 KV cache 是 GB 级 HBM 常驻数据，绑在某张特定 GPU 上。朴素旋转等于每个 step 都付 HBM 间传输代价。Meta 的 pass-Q 变体存在的全部原因就是回避这个：旋转 Q（每请求 1 token，便宜），不旋转 KV（很大，贵）。
> 2. **连续批处理冲突**。CP 通信组是静态的；生产 decode batch 是动态的 —— 每步 batch 是不同请求在不同阶段的不同组合。把静态 CP 组与动态批组成协调，工程上不简单。这正是 vLLM 的 CP RFC 关了一年的原因 —— 最终 shipping 路径（PCP，PR #28718）通过只针对 *prefill*（per-request batch 结构更统一）绕开了这个冲突。
> 3. **Decode 是内存带宽瓶颈**。[[ring-attention|Ring Attention]] "通信免费"前提是 $T_{\text{compute}} \geq T_{\text{comm}}$。Decode 每步计算极小（1 个 query token × full KV，全是内存读）。Meta 论文 Table 6 实测：Llama 3 405B / TP8 上 TTIT *随 CP 单调上升* —— CP1 46.26 ms → CP4 71.31 ms。
> 4. **每步通信代价**。训练把一次 ring 旋转摊销在 N 个 attention block 上；decode 把一次 ring 旋转摊销在 **每步 1 个微小 attention** 上。通信开销随 decode step 数线性增长。
>
> **Meta 的破局**：[arXiv:2411.01783](https://arxiv.org/abs/2411.01783)（Yang et al., 2024-11）引入 **pass-KV**（full prefill，KV 旋转）+ **pass-Q**（持久 KV prefill 和 decode，Q 旋转）。决策（论文 Algorithm 1）：`T + P ≥ 2·N_KV / N_H` 时用 pass-KV，否则 pass-Q。论文自己建议 **prefill 用 CP、decode 切换并行方案** —— 即与 [[prefill-decode-disaggregation|PD 分离]] 天然配对。
>
> **Gemini / Claude / GPT-4 用什么不公开** —— Gemini 1.5 tech report（[arXiv:2403.05530](https://arxiv.org/abs/2403.05530)）、Anthropic / OpenAI 披露、Google 研究员发文都没提 CP。本节早期版本写过 "Gemini-style 1M+ 推理用 CP"，现已更正为 "未证实猜测"。
>
> **2026 当前的长上下文推理 consensus**：混合，没有单一答案。128K → TP + KV 量化。512K–1M prefill → CP（Meta 路径 / vLLM PCP / TRT-LLM Ulysses）或 PP（SGLang 通用路径）。1M+ decode → TP + DCP（去重）+ PD 分离 + KV 压缩（MLA / 量化 / 淘汰 / 层次卸载），Helix-on-MNNVL 是 GB200/NVL72 时代的新选项。

> [!question]+ Shiki —— CP 不是针对 prefill 的吗，vLLM 的 decode CP 是什么，TRT-LLM 为什么要 CP，inference 主要不是 TP 吗？（2026-05-20）
>
> **直觉是对的**。"CP" 在推理世界里被**过载**成三种完全不同的东西，只有一种是 Meta paper 那个意义：
>
> **1. "真正的" CP（Meta paper 意义）** —— 沿 sequence 维度切 attention 计算，跟训练时的 [[ring-attention|Ring Attention]] 同一个东西。推理里用于 **长上下文 prefill**（128K+ token）。Decode 时论文自己（Table 6）证明 per-step CP **拖慢**：TTIT 随 CP 单调上升，因为每个 decode step 都付完整的 ring rotation 代价但 attention 极小。论文建议：*"CP best suited for improving prefill performance"* 和 *"switch parallelism scheme for decode"*。
>
> **2. vLLM 的 "Decode Context Parallel"（DCP）** —— **不是真正的 CP**，是一个起名误导性极强的 **KV cache 去重 trick**。场景：MLA 模型（DeepSeek-V3/V4）只有 1 个 KV head。为了跑 TP=8 切权重，只能 **把完整 KV cache 在 8 张 TP rank 上复制**（1 KV head ÷ 8 = 切不动）。8× 显存浪费。**DCP 把那 8 张已经存在的 TP rank 之间按 token 维度切 KV cache**，每张卡只存 1/8 的 token KV，attention 时按需 AllGather。机制：
>
> ```
>   没 DCP（TP=8、MLA）：                启用 DCP（-dcp 8）：
>   GPU 0: [KV: token 0..N]              GPU 0: [KV: token 0..N/8]
>   GPU 1: [KV: token 0..N]   ← 重复     GPU 1: [KV: token N/8..2N/8]   ← 分块
>   GPU 2: [KV: token 0..N]   ← 重复     GPU 2: [KV: token 2N/8..3N/8]
>   ...                                   ...
>   GPU 7: [KV: token 0..N]   ← 重复     GPU 7: [KV: token 7N/8..N]
> ```
>
> DCP **不是独立的并行轴** —— 它在 TP 通信组内部，不增加 GPU，也不切 attention 计算（attention 还是 TP 在做）。生效条件是 `tp_size > num_kv_heads`（MLA、MQA 受益；GQA 8 KV head + TP=8 没意义）。**更准确的命名应该叫 "MLA KV-cache sharding"**；"Decode Context Parallel" 是 marketing 病。
>
> **3. TensorRT-LLM 的 `context_parallel_size`** —— 这个 **是** 真正的 CP（Meta paper 意义），跟 TP / PP / DP / EP 平级的一等公民 API。公开文档只一句："Context parallelism distributes the processing of long sequences across multiple GPUs. Best for: Long context scenarios."。源码（`tensorrt_llm/mapping.py:25-34`）其实在同一个旋钮后面挂了 **4 个变体**：`ULYSSES`（默认，DeepSpeed-Ulysses 风格 head all-to-all）、`STAR`（Star Attention，稀疏 / anchor-block）、`HELIX`（Helix Parallelism，MNNVL 加速 decode）、`RING`（仅枚举，无代码路径）。逐个拆解见下一个 Q&A。NVIDIA 做这个一是因为 Meta 这种客户需要 Llama 3 405B 上的 1M token prefill（Meta 论文实测就在 128 张 H100 上跑），二是 Helix 给了他们 GB200/NVL72 上 decode-time CP 的故事。
>
> **你说 TP 主导推理的直觉是对的：**
>
> | 阶段 | 主流并行 | CP 角色 |
> | ---- | ------- | ------ |
> | Prefill，普通长度（≤ 128K） | TP | 不用 |
> | Prefill，长上下文（128K–1M） | TP × CP | **真正的 CP**（Ring / Ulysses）派上用场 |
> | Decode | TP（+ MoE 的 DP attention） | 几乎不用 —— per-step CP 代价高，Meta 论文 Table 6 证明 |
>
> 所以以后看到推理场景里的 "CP"，先分辨是 (1) Meta paper 那种长上下文 prefill 用的真 CP、(2) vLLM 的 DCP 其实是 KV 去重、还是 (3) TRT-LLM 的 API 旋钮其实就是变体 1。三个都顶着同样两个字母。

> [!question]+ Shiki —— TRT-LLM 的 CP 实现了什么变体，为什么 TRT-LLM 出 4 个而 vLLM/SGLang 各出 1 个，以及 Meta 算法被人用了吗？（2026-05-20）
>
> 三个问题，一条 TRT-LLM 主线。短答案：TRT-LLM 出了 **4** 个 `CpType` 变体（3 真 1 占位）；OSS 引擎各出 **1** 个（vLLM PCP、SGLang DSV4-NSA-CP），受客户 / 人力 / 硬件三方面驱动；**Meta 论文的 pass-KV/pass-Q 算法 2026 年仍是单客户技术** —— 每个主流引擎都出了 *某种* CP，但没人复制 Meta 的精确 kernel。
>
> **（1）TRT-LLM 出了什么 —— 4 个 `CpType` 变体。** 翻了本地 TRT-LLM repo（`tensorrt_llm/mapping.py:25-34`）。源码里 `CpType` 枚举有 **4 个值**，全部通过 `cp_config={"cp_type": ...}` 暴露。默认 `ULYSSES`。三个是真实现，一个是占位。
>
> | `cp_type` | 状态 | 算法 | 代码位置 |
> | --------- | ---- | ---- | -------- |
> | `ULYSSES` | ✅ 默认 | DeepSpeed-Ulysses 风格：attention 层把 CP rank **折叠成 TP**（`attn_tp_size = tp_size × cp_size`、`attn_cp_size = 1` —— `mapping.py:94-97`）。head 维度 all-to-all 通过折叠出来的 TP 通信组隐式做。 | `_torch/distributed/communicator.py:150,700`（`has_cp_ulysses`、`broadcast_cp`）。硬约束：Ulysses-CP 不能与 TP 同时启用（`communicator.py:688` 写死了）。 |
> | `STAR` | ✅ 真实现 | Star Attention（Shao et al., NVIDIA, 2024 —— arXiv:2411.17116）：上下文分块，**anchor block**（block 0）复制到所有 CP rank；每个 rank 在 `[anchor ‖ own_block]` 上做 prefill；最后 query 汇总走全局 softmax。稀疏、近似，但便宜。 | `_torch/pyexecutor/request_utils.py:160 partition_context_for_star_attention`、`:339 merge_star_attention_requests`；`_torch/pyexecutor/model_engine.py:3187 _prepare_star_attention_inputs`。需要 `attn_backend="FLASHINFER_STAR_ATTENTION"`。例子：`examples/llm-api/star_attention.py`。 |
> | `HELIX` | ✅ 真实现 | Helix Parallelism（NVIDIA）：decode 阶段 KV cache **按 token 切**到 CP rank，用 **MNNVL**（多节点 NVLink）硬件加速 `all_to_all`。非 attention 层把 CP rank **重新征用为 TP**（`mapping.py:533 repurpose_helix_cp_to_tp`）。 | `_mnnvl_utils.py:359 HelixCpMnnvlMemory`、`_torch/distributed/ops.py:366 HelixAllToAllNative`、自定义 C++ op `trtllm::alltoall_helix`、`trtllm::helix_post_process`。`_torch/modules/attention.py:437-439` 在新 PyTorch backend 里，只要 `cp_size > 1` 走到这里就 assert `cp_type == HELIX` —— **不是说 "只有 Helix 才是真 CP"**，而是因为 Ulysses 在 `mapping.py:94-97` 已经把自己折叠成 TP 了，Star 在请求调度层就处理完了，到 `Attention.forward` 时还保留 `attn_cp_size > 1` 的就只剩 Helix。 |
> | `RING` | 🟡 仅枚举 | 保留名字。`CpType.RING = "RING"` 在 `mapping.py:30-31`，但 `grep -rn 'CpType.RING'` 全仓 **零命中**。没有 dispatch，没有 kernel。 | — |
>
> **三个已实现的变体都是真 CP**，只是落在不同抽象层：
> - **Ulysses** 在 **TP 通信组里** 做 CP（head 维 all-to-all 复用 TP 通信器）。用途：单节点多卡的长 prefill。
> - **Star** 在 **请求调度层** 做 CP（上下文切块发到 cp_size 张卡，每卡跑标准的局部 attention）。稀疏近似，针对 128K-512K 的 needle-in-a-haystack 检索。
> - **Helix** 在 **attention kernel 内部** 做 CP（token 切 KV + MNNVL alltoall）。这是 *decode 阶段* CP 路径 —— 就是 Meta 论文说 decode 不该用 CP 的那种事，但 Helix 用 MNNVL `all_to_all`（NVL72 级 fabric）+ FFN 把 CP rank 重新征用为 TP 来绕开 per-step cost。
> - **Ring** 没实现 —— 可能是给未来 pass-KV / pass-Q kernel 的位置，或者跟训练侧 Ring Attention 词汇对齐。
>
> 所以 "真 CP" 不是 Helix 专属标签 —— Ulysses 和 Star 也都是真 CP，只是被 relabel 了或者移到了更上层。公开文档一句话（*"Context parallelism distributes the processing of long sequences across multiple GPUs"*）实际把三种完全不同的算法藏在同一个旋钮后面。
>
> **（2）TRT-LLM 和 vLLM/SGLang 怎么比 —— 2026-05 引擎矩阵：**
>
> | 引擎 | Prefill CP | Decode CP | 变体数 |
> | ---- | ---------- | --------- | ------ |
> | **TRT-LLM** | Ulysses + Star | Helix | **4**（含 `RING` 仅枚举占位） |
> | **vLLM** | PCP（PR [#28718](https://github.com/vllm-project/vllm/pull/28718)，2025-11-19） | DCP（PR #24864，KV 去重 trick） | **2**（PCP + DCP） |
> | **SGLang** | DSV4 的 NSA Prefill CP（`--enable-nsa-prefill-context-parallel`） | — | **1**（DSV4-scoped） |
>
> **（3）为什么 TRT-LLM 出 4 个而 vLLM/SGLang 各出 1 个 —— 四个驱动因素：**
>
> 1. **客户结构**：NVIDIA 的核心客户（Meta 的 Llama 405B 1M-prefill、OpenAI / Anthropic 内部、NVIDIA 自家实验室）就是长上下文推理的最大需求方。TRT-LLM 定位是为他们准备的"性能旗舰"引擎。vLLM 和 SGLang 服务的是更广的 OSS 用户群，中位用例是 4K-32K，长上下文是小众 —— 一个好的 CP 变体就够。
> 2. **硬件展示 / 绑定**：Helix 在 **MNNVL** fabric（NVL72 / GB200）上才真正划算。NVIDIA 有商业动机出"只有自家旗舰硬件才跑得好"的算法 —— 卖更多 NVL72 机柜。vLLM 和 SGLang 追求硬件可移植（H100 / A100 / MI300 都得跑），所以避开拓扑强耦合的算法。
> 3. **工程资源**：TRT-LLM 背后是 NVIDIA 几百号工程师；vLLM 约 30 个活跃 maintainer，SGLang 约 10 个。维护 4 条 CP 代码路径（各自的 kernel、调度器、KV cache 配置）成本很高 —— OSS 项目能负担 1 条，不能负担 4 条。
> 4. **OSS 的克制**：
>    - **vLLM**：早期 prefill-CP RFC [#22693](https://github.com/vllm-project/vllm/issues/22693)、[#26133](https://github.com/vllm-project/vllm/issues/26133) 都被 close 成 "not planned"，但 PR [#28718](https://github.com/vllm-project/vllm/pull/28718)（PCP）还是在 2025-11-19 走直接 PR 路径合并了。他们走 *最小* 路径：复用 DCP 的 KV cache interleaving 基础设施，出一个 prefill-CP 变体（Ulysses-leaning，AllReduce + ReduceScatter 通信模式），收工。
>    - **SGLang**：[LMSYS 博客 2026-01-15](https://www.lmsys.org/blog/2026-01-15-chunked-pipeline/) 拒绝了 *通用* CP —— "CP 要侵入性的 per-model 修改"。但 DeepSeek-V4 的 NSA attention 要求它时，他们做了 *DSV4-scoped* prefill CP（round-robin token 切分 + `cp_all_gather`）。哲学：不做通用 CP 层，但 attention 变体要求时做 per-model CP。
>
> **（4）Meta 的精确算法被采用了吗？**
>
> | 谁 | 状态 |
> | --- | ---- |
> | **Meta 自己** | ✅ Llama 405B 1M-prefill 服务（[论文](https://arxiv.org/abs/2411.01783) 本质上就是他们的生产部署 writeup；128 张 H100 上 77 秒）。|
> | **TRT-LLM** | ❌ **没有**实现 Meta 的 pass-KV / pass-Q。出的是 Ulysses + Star + Helix —— 同一个问题，完全不同的算法。|
> | **vLLM** | ✅ PCP 2025-11-19 合并 —— 自己的算法（AllReduce + ReduceScatter，Ulysses-leaning），**不是** Meta 的 pass-KV / pass-Q。|
> | **SGLang** | ✅ DSV4 的 NSA Prefill CP —— round-robin token + `cp_all_gather`，**不是** Meta 算法。scope 限于一个模型。|
> | **Anthropic / OpenAI / Google** | 未公开 —— 推测是 KV 压缩 + PP / chunked prefill，而不是 Meta 风格 CP。|
>
> 为什么 Meta 的精确技术没扩散：
>
> 1. **没开源参考实现**：论文 2024-11 发出来了，inference engine 没开。要采用就得从论文 Algorithm 1（pass-KV vs pass-Q 决策逻辑）从头实现。
> 2. **引擎集成成本**：pass-KV 还是 pass-Q 是个 *逐步* 决策，依赖当前 cache 状态和 query batch size。主流引擎缺这层抽象 —— 它们的 CP 通信组是静态的。
> 3. **场景太窄**：只在 512K-1M 上下文才划算。多数生产负载是 4K-128K，TP / DP / chunked prefill 已经够用。
> 4. **硬件复刻难**：论文假设 128 张 H100 的拓扑，搬到 B200 / H200 / GB200 不是 copy-paste —— 通信/计算比例会变，Algorithm 1 的最优决策边界也会移动。
>
> **结论**：Meta 论文 *有影响力* —— 它确立了"推理 CP 行得通、值得为它做工程"的框架。到 2026 年每个主流引擎都出了某种受这个框架启发的 CP 变体（vLLM PCP、SGLang NSA-CP、TRT-LLM Ulysses/Star/Helix）。但论文的 *具体算法*（pass-KV / pass-Q 混合）仍然是只有 Meta 自己在用的 **单客户技术** —— 其他人都在同一个名字下用了不同算法。论文的 *思想* 影响整个领域，但论文的 *具体 kernel* 没什么人用。

> [!question]+ Shiki —— CP 为什么做不到通用？训练侧不也是要针对每个模型单独写吗？（2026-05-20）
>
> SGLang 那句 "CP 要侵入性的 per-model 修改" 是真的，但训练 vs 推理确实是 *不对称的*。差别在于 **CP 接入到哪一层抽象**。
>
> **训练侧 —— CP 看起来通用，因为 attention kernel 是统一的。**
>
> - 框架（Megatron-LM、DeepSpeed、PyTorch native CP）把 attention 当**黑盒**：`attn(Q, K, V, mask) → O`。标准 [[ring-attention|Ring Attention]] 和 [[deepspeed-ulysses|DeepSpeed Ulysses]] kernel（FlashAttention-2/-3）覆盖绝大部分训练模型。
> - CP 在 **transformer-block 层** 包装：沿 sequence 切 Q/K/V → 通信 → 调标准 attention kernel → 合并输出。这层 wrapper 只用写一次，**所有用标准 transformer block 的模型自动受益**。
> - 训练 **没有 KV cache 管理** —— 一大堆 per-model 复杂度直接缺席。
> - Megatron *端到端重写模型*（自定义 transformer block、自定义 attention 调用点）。Per-model 差异在 *框架自己的模型代码里* 消化，不是在 CP 边界上。
>
> **推理侧 —— CP 必须 per-model，因为 attention kernel 是异构的。**
>
> - vLLM / SGLang **不重写模型**，得适配 HuggingFace 千奇百怪的 attention 实现：
>   - MHA / MQA / GQA / **MLA**（DeepSeek-V3/V4）/ **NSA**（DSV4 稀疏）/ **sliding window**（Mistral）/ **hybrid local/global**（Gemma 3）/ **sink tokens**（Llama 4）/ **gated delta attention**（RWKV 系）...
>   - 每种 attention 变体有自己的 PagedAttention / FlashInfer / Triton kernel，*接口都不一样*。
> - CP 需要 **嵌入 / 邻接** 每个 attention kernel —— 得知道 KV-cache layout、page table 格式、masking 约定。这些跨模型完全不标准。
> - **KV cache 管理** 又加了一根 per-model 轴。MLA 的压缩 KV 跟 GQA 的标准 KV 长得完全不像。PCP / DCP 要分别处理每种 layout。SGLang 出 `--enable-nsa-prefill-context-parallel` 就是因为 NSA 的 KV 格式跟通用 prefill-CP 路径对不上。
>
> **一句话总结**：训练 CP 接在 *transformer-block* 层（标准接口、通用适配），推理 CP 必须接在 *attention-kernel* 层（每个模型有自己的 kernel）。这就是为什么训练侧"框架支持后就免费了"，而推理引擎每出一个新 attention 变体就得重做一遍。
>
> **不过这个 gap 在缩小。** 训练侧也越来越多 model-specific kernel —— FlashAttention-3 有 NSA 风格变体，MLA 需要自己的训练 kernel，DeepSeek-V4 的 NSA 训练要自定义 forward/backward。所以"训练 CP 通用"这个优势在淡化。到 2027 年训练 vs 推理在 attention kernel 多样性上的不对称会小得多。
