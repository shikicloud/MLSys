...
> 是的，部分支持，但分化明显。**引擎矩阵：**
>
> - **vLLM**：出了 *Decode Context Parallel*（DCP，`--decode-context-parallel-size N`），但这是一个窄义的 **KV cache 去重技巧** —— 针对 MLA 类模型 `tp_size > num_kv_heads` 导致 KV 在 TP rank 间复制的情况，把 KV 沿 token 维度切到 TP 通信域里。**不是**通用的长序列并行。完整 prefill CP 的 RFC（[#22693](https://github.com/vllm-project/vllm/issues/22693)、[#26133](https://github.com/vllm-project/vllm/issues/26133)）都被关了 "not planned"；只有 [#30055](https://github.com/vllm-project/vllm/issues/30055)（DeepSeek DSA）还活着。GQA 的 DCP PR [#24864](https://github.com/vllm-project/vllm/pull/24864) 2025-10-14 合并。
> - **SGLang**：明确选了 **流水线并行** 而不是 CP 来做百万 token 服务。[LMSYS 博客（2026-01-15）](https://www.lmsys.org/blog/2026-01-15-chunked-pipeline/) 给出理由：CP "需要针对每种模型变体做特定的、通常侵入性的 attention 机制修改"；PP 的通信量更低。结果：Qwen3-235B / H20 上 1M TTFT 420 秒。CP feature request [#12196](https://github.com/sgl-project/sglang/issues/12196) 2025-10 提的，没人接。
...
> 2. **连续批处理冲突**。CP 通信组是静态的；生产 decode batch 是动态的 —— 每步 batch 是不同请求在不同阶段的不同组合。把静态 CP 组与动态批组成协调，工程上不简单。这正是 vLLM 把 CP RFC 关成 "not planned" 的原因。
...
>
> **2026 当前的长上下文推理 consensus**：混合，没有单一答案。128K → TP + KV 量化。512K–1M prefill → CP（Meta 路径）或 PP（SGLang 路径）。1M+ decode → TP + DCP（去重）+ PD 分离 + KV 压缩（MLA / 量化 / 淘汰 / 层次卸载）。
...
>
> 两个相关问题，都关乎推理 CP 的 *部署现实*。短答案：**TRT-LLM 是异类**，**Meta 的论文在 2026 实际上是单客户技术**。
>
> **为什么 TRT-LLM 出 4 个变体而 OSS 引擎不跟 —— 四个驱动因素：**
>
> 1. **客户结构**：NVIDIA 的核心客户（Meta 的 Llama 405B 1M-prefill、OpenAI / Anthropic 内部、NVIDIA 自家实验室）就是长上下文推理的最大需求方。TRT-LLM 定位是为他们准备的"性能旗舰"引擎。vLLM 和 SGLang 服务的是更广的 OSS 用户群，中位用例是 4K-32K，长上下文是小众。
...
> 3. **工程资源**：TRT-LLM 背后是 NVIDIA 几百号工程师；vLLM 约 30 个活跃 maintainer，SGLang 约 10 个。维护 4 条 CP 代码路径（各自的 kernel、调度器、KV cache 配置）成本很高 —— OSS 项目负担不起。
> 4. **OSS 的显式取舍**：
>    - **vLLM**：prefill-CP 的 RFC [#22693](https://github.com/vllm-project/vllm/issues/22693)、[#26133](https://github.com/vllm-project/vllm/issues/26133) 都被 close 成 "not planned"。他们优先 **DCP**（MLA KV cache 去重）—— 因为这是 vLLM 典型用户（TP=8 跑 DeepSeek-V3/V4 级 MLA 模型，KV 复制 8×）的真痛点。
>    - **SGLang**：百万 token 服务明确选 **PP** 而不是 CP。LMSYS 博客（2026-01-15）："CP requires intrusive per-model modifications to the attention mechanism; PP has lower comm volume." 他们的押注是 pipeline 深度 + chunked context > sequence sharding。
>
> **Meta 的 CP 真的被部署了吗？**
>
> | 谁 | 状态 |
...
> | **vLLM** | ❌ Prefill-CP RFC 都关了；只出了 DCP（KV 去重）。|
> | **SGLang** | ❌ 明确选了 PP。|
...
>
> **结论**：Meta 论文 *有影响力* —— 它确立了"推理 CP 行得通、值得为它做工程"的框架。但 2026 年它实际上是 **单客户技术**：Meta 在用；其他人要么跳过 CP（vLLM、SGLang），要么用同一个名字下完全不同的算法（TRT-LLM 的 Ulysses/Star/Helix）。论文的 *思想* 影响整个领域，但论文的 *具体算法* 没什么人用。
