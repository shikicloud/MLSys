...
> [!abstract]+ TL;DR
> Ring Attention 把 attention 的序列维度切到 $N$ 张 GPU 上排成 **环**：每张 GPU 保留一份固定的本地 Q 块，K/V 块沿环 **旋转**。$N{-}1$ 轮旋转之后每个 Q 都跟每个 K、V 配对过 —— 得到 **与单卡 attention 完全相同的输出**，但每卡 activation 内存是 $O(b \cdot c \cdot h)$，**与总序列长度无关**。技术创新点是把这套分布式旋转跟 **FlashAttention 风格的 streaming softmax** 融合：每个 Q 行维护 `(max, sum, output)` 三元组在所有 $N$ 个 chunk 之间累计，永远不实例化完整 attention matrix。块大小 $c \geq F/B$（host FLOPs / 互联带宽）时，KV 旋转 **跟 attention 计算完全重叠** —— 通信免费。论文报告：7B 模型 **32× A100 上 4M token context**、**TPU v4-1024 上 8M token**。今天的生产落地：**Megatron-Core Context Parallelism、Meta Llama 3 训练（4D 并行配 CP）、Tencent USP** 都是 Ring Attention 加几年的工程修复（zigzag 调度、pass-Q 变体、与 [[deepspeed-ulysses|Ulysses]] 的混合）。裸 2023 算法不能直接用；2026 的生产栈才是算法 + 工程加固版。**对比** [[deepspeed-ulysses|DeepSpeed Ulysses]]：Ring 可以扩到任意 $N$ 但需要高带宽；Ulysses 卡在 $N \leq \text{num\_heads}$ 但每卡通信量恒定。混合（USP）是生产答案。
...
[[paged-attention|FlashAttention]] 解决了单卡内存（流式 tile 化 attention），但 $N = 1$ 时还是 $O(S/N)$ 每卡内存。自然下一步：**把序列维度本身切到多卡上**，每张卡只持有 $S/N$ 个 token。
...
- **通信在干净条件下免费**。每步计算 $4 \cdot d \cdot c^2 / F$（块 attention FLOPs / host FLOPs/s）；每步通信 $4 \cdot c \cdot d / B$（一个 KV 块 / 互联字节/s）。重叠要求 $c \geq F/B$。A100 + NVLink 上 $F/B \approx \text{几百 token}$ —— 容易满足。
...
每次旋转、每卡：$2 \cdot c \cdot d \cdot \text{dtype\_bytes}$（一个 K 块 + 一个 V 块）。**与总序列长度 $S$ 无关** —— 只看块大小 $c$。

$c = 4096$、$d = 8192$、FP16：$2 \cdot 4096 \cdot 8192 \cdot 2 = 128$ MiB / 旋转。600 GB/s NVLink：~210 µs / 旋转。本地 $c \times c$ 块 attention 计算在毫秒级 —— 通信藏住了。
...
`zhuzilin/ring-flash-attention` 和 Megatron-Core CP 用一种 **zigzag** 方案做同样的均衡。2026 生产级 Ring Attention 总是配一种这样的修复。
...
这些都是 *训练* 上下文长度，数学上与单卡 attention 等价 —— 没有质量妥协。

### 下游生产示范：Large World Model (LWM)

[Liu, Yan, Zaharia, Abbeel, *World Model on Million-Length Video And Language with Blockwise RingAttention*](https://arxiv.org/abs/2402.08268)（2024-02）。7B 模型从 4K 渐进训到 **1M token context**。技术最有说服力的端到端演示。

### 生产部署

| 部署 | 变体 | 来源 |
| ---- | ---- | ---- |
| **Megatron-Core CP** (NVIDIA) | Ring + cuDNN FlashAttention + zigzag 调度、4D 并行 (TP × CP × PP × DP) | [docs.nvidia.com/megatron-core/.../context_parallel.html](https://docs.nvidia.com/megatron-core/developer-guide/0.16.0/user-guide/features/context_parallel.html) |
| **Meta Llama 3 训练** | "pass-KV"（经典 Ring）训练 + "pass-Q" 变体用于持久 KV cache 解码。**Llama 3 405B 在 128× H100 上 77 秒做 1M token prefill，93% 并行效率，63% FLOPs 利用率** | [arXiv:2411.01783](https://arxiv.org/abs/2411.01783) |
| **Tencent USP** | Ring × [[deepspeed-ulysses\|Ulysses]] 混合 2D mesh；47% MFU 训 Llama-3-8B 在 2×8×A800 上 208K | [arXiv:2405.07719](https://arxiv.org/abs/2405.07719)、[feifeibear/long-context-attention](https://github.com/feifeibear/long-context-attention) |
| **PyTorch 原生 Context Parallel** | Ring 为主，All-to-all 传输为次（PyTorch 2.7+） | [docs.pytorch.org/.../context_parallel.html](https://docs.pytorch.org/tutorials/unstable/context_parallel.html) |
...
最强两点：(1) **跟单卡 attention 数学等价** —— 不近似、不折损质量；(2) **内存扩展性根本不同** —— 每卡 activation 内存与 $S$ 无关，序列长度随设备数线性扩展而单卡压力不变。
...
- **推理 / 解码很别扭**。自回归生成时持久 KV cache 不能随意旋转。Meta 的 pass-Q 变体就是为此设计的 —— 朴素 Ring 假设 KV 可旋转，解码时不成立。
...
[lhao499/RingAttention](https://github.com/lhao499/RingAttention) —— Apache-2.0，参考实现。关键文件：
