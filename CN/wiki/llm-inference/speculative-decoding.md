...
> [!abstract]+ TL;DR
> 自回归 LLM 解码是**内存带宽受限**的 —— 等待权重加载时 GPU 算力严重浪费。投机解码利用这一点：一次前向传播验证 $K$ 个候选 token 的成本约等于生成 1 个，所以用轻量级草稿机制提出 $K$ 个 token，目标模型用拒绝采样并行验证。**输出分布与目标模型数学上完全一致** —— 无损加速。由 Leviathan et al.（2023）和 Chen et al.（2023）独立提出。当前 SOTA：**EAGLE-3**（NeurIPS 2025）3–6× 加速；生产部署在 [[vllm|vLLM]] 和 [[sglang|SGLang]] 中。
