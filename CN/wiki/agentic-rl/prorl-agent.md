...
trainer 的 API 契约是 `① POST /add_llm_server → ② POST /start → ③ POST /process { instance, sampling_params }`（阻塞）`→ ④ ← (token_ids, logprobs, reward, timing)`。下图只做结构映射；每个组件的详细解释在 [[#系统是如何工作的]] 各小节。
...
flowchart TB
    ...
    subgraph Server["ProRL Agent Server"]
        direction TB
        FastAPI["<b>FastAPI 父进程</b><br/>futures: job_id → Future<br/>+ response_listener 后台线程"]:::svc
        MQ["3 × multiprocessing.Queue<br/>(request / result / control)"]:::state
        Worker["<b>server_worker</b>（mp 子进程, os.setsid）<br/>OpenHandsServer + ThreadPoolExecutor"]:::svc
        INIT["<b>INIT</b> · N 个 worker<br/>构建 .sif, 装依赖"]:::svc
        RUN["<b>RUN</b> · N 个 worker<br/>多轮 agent 循环, LLM ↔ 工具"]:::svc
        EVAL["<b>EVAL</b> · N 个 worker<br/>给轨迹打分, 奖励函数"]:::svc
        AHandler["<b>AgentHandler</b> 按 data_source 分派<br/>.init / .run / .eval / .final_result"]:::state
        State[("per-job: JobDetails · PausableTimer · Event<br/>共享: weighted_addresses (min-heap LB)")]:::state

        FastAPI <--> MQ
        MQ <--> Worker
        Worker --> INIT
        INIT -->|"job_id (init 完成)"| RUN
        RUN -->|"job_id (释放容器)"| EVAL
        EVAL --> AHandler
        Worker -.使用.-> State
    end

    Sandbox["<b>Singularity 沙箱</b>（每任务）<br/>rootless · --fakeroot · --network none<br/>per-job 127.x.x.x · ptyprocess bash · IPython"]:::ext
    vLLM["<b>vLLM 后端池</b><br/>按在飞计数 min-heap 负载均衡<br/>单次 rollout → 同一后端（前缀缓存亲和）"]:::ext

    Trainer <-->|"HTTP /process（阻塞）"| Server
    Worker --> Sandbox
    Worker --> vLLM
```

**怎么读这张图。** trainer 只通过 HTTP 与 server 通信（顶端）。server 内部 FastAPI 是一个薄父进程，通过 `multiprocessing.Queue` 把请求转发给真正干活的子进程；子进程跑三阶段流水线（`INIT` → `RUN` → `EVAL`），每个阶段有自己的队列和 worker 池；worker 通过 `AgentHandler`（按 `data_source` 选）分派。同一个 worker 在底端连出两个外部资源：每任务一个 Singularity 沙箱负责工具执行，一个负载均衡的 vLLM 后端池负责 LLM 调用。每阶段的具体机制、内核约束、负载均衡数学都在下面的子节里。
