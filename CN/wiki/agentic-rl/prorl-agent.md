...
下面用 Mermaid。trainer 的 API 契约是 `① /add_llm_server → ② /start → ③ POST /process { instance, sampling_params }`（阻塞）`→ ④ ← (token_ids, logprobs, reward, timing)`。每个组件的详细解释在 [[#系统是如何工作的]] 各小节，本图只做结构映射。
...
flowchart TB
    ...
    subgraph Server["ProRL Agent Server"]
        FastAPI["FastAPI 父进程<br/>+ futures 表<br/>+ response_listener"]:::svc
        MQ["3 × multiprocessing.Queue"]:::state
        Worker["server_worker（mp 子进程）<br/>OpenHandsServer +<br/>ThreadPoolExecutor"]:::svc
        FastAPI <--> MQ <--> Worker
    end

    subgraph Pipeline["三阶段流水线（worker 内）"]
        direction LR
        INIT["<b>INIT</b><br/>构建 .sif"]:::svc
        RUN["<b>RUN</b><br/>多轮循环"]:::svc
        EVAL["<b>EVAL</b><br/>打分 / 奖励"]:::svc
        INIT -->|"job_id"| RUN -->|"job_id"| EVAL
    end

    AHandler["AgentHandler 分派<br/>(按 data_source)"]:::state
    State[("per-job: JobDetails · timer · event<br/>shared: min-heap LB")]:::state
    Sandbox["Singularity 沙箱<br/>(rootless · --fakeroot ·<br/>per-job 127.x.x.x)"]:::ext
    vLLM["vLLM 池<br/>(min-heap LB · 粘性)"]:::ext
...
**怎么读这张图。** trainer 只通过 HTTP 与 server 通信。server 内部 FastAPI 是一个薄父进程，通过 `multiprocessing.Queue` 把请求转发给真正干活的子进程；子进程跑三个队列各自独立的 worker 池，通过 `AgentHandler` 分派，最后接到两个外部资源（每任务的沙箱 + 负载均衡的 vLLM 池）。每阶段的具体机制、内核约束、负载均衡数学都在下面的子节里。
