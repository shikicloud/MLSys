...
## 这怎么改变了 ProRL Agent vs NeMo Gym 的图

[昨天 wiki 的那节](#相关阅读)把 [[prorl-agent|ProRL Agent]] 和 [[nemo-gym|NeMo Gym]] 对比成"同族、不同层，没有公开 adapter 桥"。**Polar 就是那座桥**，且在那节墨迹未干时已经出货了。更新后的图：

```
Trainer (NeMo RL / VeRL / Slime / Unsloth)
    │
    │  异步 rollout-as-a-service 契约          ← Polar 的 gateway endpoints
    ▼
Polar  (rollout server + gateway 节点)
    │
    │  小 harness adapter（配置 + shell 命令）
    │  + provider-API proxy（Anthropic / OpenAI / Google）
    │
    ▼
未修改的 agent harness（Codex / Claude Code / Qwen Code / Pi / ...）
    │
    │  注册为以下之一 ──────────────────────────┐
    ▼                                          │
NeMo Gym 环境                                  │
...
所以：
- **ProRL Agent** 现在是 legacy —— 同一个 NVIDIA-NeMo/ProRL-Agent-Server repo，但代码已被重写
- **Polar** 是当前生产 rollout 驱动层
- **NeMo Gym** 仍然拥有环境 catalog（84 benchmark、19 harness）—— Polar 注册为它的一个环境

[[prorl-agent#ProRL Agent vs NeMo Gym —— 同族、不同层|ProRL Agent 对比节里]]的"必须二选一时"现在的答案是：**两个都用**，Polar 当 rollout 层。
