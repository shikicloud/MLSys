...
> [!abstract]+ TL;DR
> 环境设计决定智能体 RL 能学到什么、学得多好、能否泛化 —— 必须平衡**高保真度**（接近真实）、**高效率**（可大规模并行）、**安全性**（隔离危险操作）、**多样性**（多任务多场景）。**大脑 vs. 身体**架构解耦 LLM 策略与沙箱执行。2025–2026 出现基础设施平台：**OpenReward**（330+ 环境 API，ORS 标准扩展 [[mcp-protocol|MCP]] 用于 RL）、**ARES**（Martian，RL 优先的编程智能体）、**Daytona**（亚 90ms 沙箱创建）。环境 scaling law 与模型 scaling law 对应 —— 更大模型需要更多样的训练环境。
