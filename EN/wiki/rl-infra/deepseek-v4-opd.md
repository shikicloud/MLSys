...
# Figures TODO: tech report PDF (HF only) has no OPD-specific figures; pdfimages yielded only fragments of Fig. 1 (benchmark bar chart). No pipeline / KL-comparison / cache-infra diagrams in the report.
---
...
**What it is.** DeepSeek-V4 (1.6T-parameter MoE, 49B active) makes one critical methodological substitution vs V3.2: per §5.1 of the tech report, *"the mixed Reinforcement Learning (RL) stage was entirely replaced by On-Policy Distillation (OPD)."* The post-training pipeline becomes **base → per-domain (SFT → GRPO) specialists → multi-teacher OPD merge → V4**.

**The one idea.** Train per-domain specialists with GRPO, then merge them into a single unified policy via on-policy distillation against $> 10$ teachers using **exact per-token full-vocabulary reverse KL**. Three pieces hold it up:

1. **Multi-teacher merging via reverse KL**, not weight averaging — specialists' behavioral fingerprints are preserved.
2. **Full-vocabulary KL** instead of the token-level Monte Carlo estimator used by TML / MiniLLM / GKD — variance-reduction lever critical at long-rollout scale.
3. **Engineering recipe** (cache teacher hidden states, sort samples by teacher, TileLang exact-KL kernel, FP4 teacher QAT, reuse V3.2 rollout / WAL infra) that makes the above feasible at 1.6T-MoE scale.

Remove any one: lose multi-teacher and you have single-teacher OPD with TML's limitations; lose full-vocab and the variance pathology returns; lose the engineering and the recipe doesn't run.

**Headline result.** V4-Pro is positioned as competitive with GPT-5.2 and Gemini-3.0-Pro on reasoning benchmarks (AIME, GPQA, code), and claims SimpleQA SOTA among open-source. **No GPU-hour comparison vs GRPO is reported** — any "10× cheaper than RL" claim circulating in V4 coverage is borrowed from TML's Qwen3 blog, not validated for V4.

**Why it matters.**

- **First flagship-scale demonstration** that OPD can entirely replace the mixed-RL merge stage in post-training. Canonical reference for 2026 onwards.
- **Full-vocabulary KL is the new default at scale.** Token-level estimator's variance grows with rollout length — the wrong scaling behavior for 16K+ token reasoning models.
- **OPD-as-merge-stage** answers a real problem (capability dilution under unified RL) that anyone training large MoE faces; expect Qwen / Mistral / open-source community to adopt in 6–12 months.
- **What it is *not*: a universal RL killer.** V4 still uses GRPO inside Stage 1. The substitution is in the *merge* stage only.

---

# Depth (drill-down starts here)

The summary above is the executive layer. Everything below is for the careful reader who wants pipeline detail, the algorithmic argument, the engineering recipe, and the scope of the empirical claims.
...
| ------- | ------------------------ | ----------------- |
| **Sparse outcome reward** | 16K+ token reasoning rollouts get one 0/1 signal | Most tokens have no credit assignment; reconstructing it from a scalar is GPU-expensive |
...
The trainer's API contract from V3.2 stays the same; what changes is what runs in the merge step.
...
> [!note] GRPO is still used — just not for merging
> V4 doesn't replace GRPO globally. It replaces the unified *mixed-RL* stage that V3.2 used to merge specialists together. Per-domain RL still happens; the merge is what becomes OPD.
...
The full-vocabulary form has been mathematically obvious since GKD (2023). Nobody used it at scale because the memory / bandwidth made it infeasible. **V4's algorithmic contribution is the choice; V4's systems contribution is making it run.**

## Infrastructure that makes full-vocab OPD feasible (§5.2.2)

This is what turns Eq. 29 from a research dream into a shipping pipeline. Five engineering moves let $10+$ teachers × full-vocab KL × 1.6T model actually run.
...
### 3. TileLang exact-KL kernel + FP4 teacher QAT

> [!note]- Async I/O, TileLang kernel, FP4 QAT, and reuse of V3.2 stack — open for the rest of the recipe
>
> **Async I/O off the critical path.** Teacher weights live in centralized distributed storage with ZeRO-like parameter sharding. Hidden-state caches similarly. Loads run async while the previous teacher chunk computes.
>
> **Custom TileLang kernel for exact KL.** TileLang is a recent NVIDIA-developed tile-based programming model (similar to Triton). V4 ships a custom kernel computing exact teacher-student KL **fused with the softmax**, avoiding the two-pass "logits then loss" pattern.
>
> **FP4 teacher QAT.** Teacher weights are **FP4-quantized via QAT** (not post-training quantization) to fit in storage and make on-demand loading bandwidth-feasible. Student stays full precision. Argument: teacher provides the *target distribution* (not gradients), so FP4 inference is accuracy-acceptable.
>
> **Reuse the V3.2 rollout / WAL stack.** OPD rollouts ride on the **same preemptible, fault-tolerant WAL-based generation service that handles GRPO** in Stage 1. From the infrastructure perspective, OPD looks like RL minus the reward — same scheduler, same rollout engine, same fault tolerance.
...
- **Reasoning quality**: V4-Pro is competitive with GPT-5.2 and Gemini-3.0-Pro on AIME, GPQA, code. V4-Pro claims SimpleQA SOTA among open-source models.
- **Architecture**: 1M context with hybrid CSA + HCA attention, mHC residuals, Muon optimizer — but the architecture story is separate from the OPD story.
- **OPD pipeline validation**: the merge produces a single unified model that the paper claims preserves the per-domain specialist capabilities.

**What the paper does NOT report** (and this is what matters for whether to believe the OPD pitch):
...
| Concrete $w_i$ schedule | Whether routing is learned or hand-tuned | Tech report only says "relative importance" |
...
> [!example]- How V4's OPD relates to other OPD deployments (comparison table)
>
...
2. **Full-vocabulary KL becomes the default for OPD at scale.** Token-level estimator's variance pathology is real and grows with rollout length — exactly the wrong scaling behavior for 16K+ token reasoning models. Once the V4 engineering recipe is replicated in HF TRL / veRL / NeMo-RL, expect token-level OPD to be relegated to small-scale prototyping.
...
> [!note]- Public release status — what's open, what isn't
>
> | Artifact | Status |
> | -------- | ------ |
...
> [!note]- Closest available reference implementations
>
