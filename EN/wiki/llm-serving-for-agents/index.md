...
- [[thunderagent]] — ThunderAgent: **ICML 2026 Spotlight (top 2.2%)** — contemporary competitor to [[continuum|Continuum]] and **system layer for [[mori|MORI]]**. First system to model multi-turn workflows as first-class **agentic programs** $P = \langle ID, c, \mathcal{T}, \mathcal{L}, \tau, s \rangle$ and schedule at program granularity rather than per-request. **Three components**: (1) periodic thrashing monitor at $\Delta t = 5$ s evaluating $C_{\text{total}} < \sum_p c_p$ on each backend; Pause/Restore primitives prioritize shortest-context-first (formalized by Lemma 4.1: recompute cost $\propto c_i^2$, so greedy shortest-first eviction is optimal); time-decay function $f(t) = 2^{-t}$ on Acting programs (proven optimal under memoryless tool durations) provides Continuum's TTL counterpart but reactive rather than predictive. (2) Global program-aware waiting queue across DP nodes (paused KV is node-agnostic, so any backend with capacity can restore); bounds cross-node memory imbalance by $c_{\min} \cdot \Delta t$ vs the 51% peak imbalance vLLM+SGLang baselines suffer. (3) Hook-based tool resource GC + asynchronous env preparation (Docker images / dependency installs start in parallel with high-priority restoring programs' LLM phase, hiding 29.9-47.2 s setup latency). **Headline**: **1.48–3.58× over vLLM** on 6 benchmarks (SWEAgent, OpenHands, ToolOrchestra, ScienceAgent × GLM-4.6 / Qwen3-235B / Qwen3-8B), **beats Continuum on all 6** including ToolOrchestra-HLE where **Continuum loses to vLLM (0.65×)** under stochastic tool durations; **1.79-3.92× RL rollout** vs vLLM+SGLang Gateway SOTA; **up to 4.2× disk savings**; ~100% KV hit rate vs Continuum's collapse to ~60% under high concurrency. **OpenAI-compatible API** with one extra `program_id` parameter; MIT license; vLLM + SGLang backends (Georgia Tech/CMU/UIUC/Together AI, arXiv 2602.13692, Feb 2026)
