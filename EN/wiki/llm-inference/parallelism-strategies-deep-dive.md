...
**Training vs inference.** CP is historically a *training* technique — gradient/activation memory scales with $S$ and overwhelms single GPUs first. In *inference*, the first answers are usually KV-cache compression ([[saw-int4|SAW-INT4]], MLA, KV pruning) and/or [[prefill-decode-disaggregation|PD disaggregation]] before reaching for CP. **Meta** is the one frontier lab that has publicly demonstrated CP at inference: ["Context Parallelism for Scalable Million-Token Inference" (arXiv:2411.01783)](https://arxiv.org/abs/2411.01783) shows Llama 3 405B doing 1M-token prefill in 77 s on 128 H100s via a **pass-KV / pass-Q hybrid** ring attention. Engine support today: **vLLM** ships both *Decode Context Parallel (DCP)* — a narrower KV-dedup variant exposed via `--decode-context-parallel-size` (PR [#24864](https://github.com/vllm-project/vllm/pull/24864), Oct 2025) — and *Prefill Context Parallel (PCP)*, merged via PR [#28718](https://github.com/vllm-project/vllm/pull/28718) on 2025-11-19. **TensorRT-LLM** exposes `context_parallel_size` as a first-class API ([docs](https://nvidia.github.io/TensorRT-LLM/features/parallel-strategy.html)). **SGLang** *also* ships prefill CP, scoped to DeepSeek-V4's NSA attention: `--enable-nsa-prefill-context-parallel` flag (multiple PRs through 2026-05, e.g. [#23882](https://github.com/sgl-project/sglang/pull/23882), [#24560](https://github.com/sgl-project/sglang/pull/24560)). Their earlier ["PP over CP" stance](https://www.lmsys.org/blog/2026-01-15-chunked-pipeline/) (Jan 2026) was about *generic million-token serving*; for DSV4 NSA specifically they did ship CP. **Other frontier labs (Google Gemini, Anthropic Claude, OpenAI GPT-4) have not publicly disclosed whether their long-context serving uses CP** — claims they do are unverified speculation.

> [!question]+ Shiki — Do vLLM / SGLang / TensorRT-LLM support CP at inference, and why was CP previously training-only? (2026-05-20)
>
> Yes, partially, with a sharp asymmetry. **Engine matrix:**
>
> - **vLLM**: now ships **both** *Decode Context Parallel* (DCP, `--decode-context-parallel-size N`) and *Prefill Context Parallel* (PCP, PR [#28718](https://github.com/vllm-project/vllm/pull/28718), merged 2025-11-19). DCP is a narrow **KV-cache deduplication trick** for MLA models where `tp_size > num_kv_heads` leaves KV duplicated across TP ranks. PCP is actual sequence-dim sharding for prefill — it reuses the DCP KV-cache interleaving infrastructure (`dcp_kv_cache_interleave_size` was generalized to `cp_kv_cache_interleave_size`), works on top of existing Triton attention backends, and supports both MLA and GQA. Notably, PCP shipped via direct PR even though the earlier RFCs ([#22693](https://github.com/vllm-project/vllm/issues/22693), [#26133](https://github.com/vllm-project/vllm/issues/26133)) were closed as "not planned" — RFC status didn't predict shipping status. The PR doesn't name a specific algorithm ([[ring-attention|Ring]] / [[deepspeed-ulysses|Ulysses]] / pass-KV-pass-Q); its comm pattern uses AllReduce + ReduceScatter (`cp_lse_ag_out_ar`), suggesting a hybrid or Ulysses-leaning rather than pure Ring.
> - **SGLang**: mixed picture. For *generic* million-token serving they chose **pipeline parallelism** ([LMSYS blog 2026-01-15](https://www.lmsys.org/blog/2026-01-15-chunked-pipeline/): CP "requires intrusive per-model modifications to attention", PP has lower comm volume — 1M TTFT 420 s on Qwen3-235B / H20). The generic Decode-CP feature request [#12196](https://github.com/sgl-project/sglang/issues/12196) (Oct 2025) is closed/inactive. **But** for DeepSeek-V4's NSA attention specifically they shipped prefill CP via `--enable-nsa-prefill-context-parallel` (multiple PRs through 2026-05, including [#23882](https://github.com/sgl-project/sglang/pull/23882), [#24560](https://github.com/sgl-project/sglang/pull/24560)). Algorithm: round-robin token slicing + `cp_all_gather` (Ulysses-leaning). Tested at B300 TP=8 CP=8. Their stance is "no generic CP, but per-model CP when the attention variant demands it."
...
>
> 1. **Persistent KV cache.** In training, KV is recomputed every forward — rotating it is free. In decode, KV cache is GB-scale HBM-resident data attached to a specific GPU. Naïvely rotating it costs HBM-to-HBM transfers every step. Meta's pass-Q variant exists precisely to sidestep this: rotate Q (1 token per request, cheap) instead of KV (massive, expensive).
> 2. **Continuous-batching conflict.** CP communicator groups are static; production decode batches are dynamic — each step's batch is a different mix of requests at different stages. Reconciling static CP groups with dynamic batch composition is non-trivial. This is exactly why vLLM's CP RFCs sat closed as "not planned" for a year — the eventual shipping path (PCP, PR #28718) skipped that conflict by targeting *prefill only*, where the batch structure is much more uniform per request.
> 3. **Decode is memory-bandwidth-bound.** [[ring-attention|Ring Attention]]'s "comm is free" condition requires $T_{\text{compute}} \geq T_{\text{comm}}$. Decode's per-step compute is tiny (1 query token × full KV, dominated by memory reads). Meta paper Table 6 shows TTIT *grows monotonically with CP degree* at decode: 46.26 ms at CP1 → 71.31 ms at CP4 on Llama 3 405B / TP8.
> 4. **Per-step communication.** Training amortizes one ring rotation across N attention blocks. Decode amortizes one ring rotation across **one tiny attention** per step. Communication overhead scales linearly with decode-step count.
>
> **Meta's resolution.** [arXiv:2411.01783](https://arxiv.org/abs/2411.01783) (Yang et al., Nov 2024) introduces **pass-KV** (full prefill, KV rotates) + **pass-Q** (persistent-KV prefill and decode, Q rotates). Decision rule (paper Algorithm 1): pick pass-KV when `T + P ≥ 2·N_KV / N_H`, otherwise pass-Q. The paper itself recommends **CP for prefill, switch parallelism scheme for decode** — i.e., natural pairing with [[prefill-decode-disaggregation|PD disaggregation]].
>
> **What Gemini / Claude / GPT-4 use is not public** — searches across Gemini 1.5 tech report ([arXiv:2403.05530](https://arxiv.org/abs/2403.05530)), Anthropic / OpenAI disclosures, and Google researcher posts return zero CP mentions for inference. Earlier wording in this section claimed "Gemini-style 1M+ inference uses CP" — corrected to "unverified speculation".
>
> **2026 consensus for 1M+ inference**: hybrid, not single-winner. 128K → TP + KV quantization. 512K–1M prefill → CP (Meta path / vLLM PCP / TRT-LLM Ulysses) or PP (SGLang path). 1M+ decode → TP + DCP (dedup) + PD disaggregation + KV compression (MLA / quantization / eviction / hierarchical offload), with Helix-on-MNNVL as the new GB200/NVL72 option.
...
>
> **1. "True" CP (Meta-paper sense)** — shards the sequence dimension during attention compute, same algorithm as training-time [[ring-attention|Ring Attention]]. Used in inference for **long-context prefill** (128K+ tokens). At decode, the paper itself (Table 6) shows per-step CP **hurts**: TTIT grows monotonically with CP degree because every decode step pays a full ring-rotation cost on a tiny per-step attention. Paper recommendation: *"CP best suited for improving prefill performance"* and *"switch parallelism scheme for decode"*.
>
...
>
> **3. TensorRT-LLM's `context_parallel_size`** — this **is** true CP (the Meta-paper sense), exposed as a first-class API alongside TP / PP / DP / EP. Public docs are one-sentence: *"Context parallelism distributes the processing of long sequences across multiple GPUs. Best for: Long context scenarios."* The source (`tensorrt_llm/mapping.py:25-34`) actually ships **four named variants** behind the same knob — `ULYSSES` (default, [[deepspeed-ulysses|DeepSpeed-Ulysses]]-style head all-to-all), `STAR` (Star Attention, sparse / anchor-block), `HELIX` (Helix Parallelism, MNNVL-aware decode), and `RING` (enum entry only, no code path). The next Q&A unpacks each. NVIDIA ships this because customers like Meta need it for 1M-token prefill on Llama 3 405B (the Meta paper itself ran on 128 H100s), and because Helix gives them a story for decode-time CP on GB200/NVL72.
>
...
> [!question]+ Shiki — Why isn't CP generic? Doesn't training also need per-model implementations? (2026-05-20)
>
> SGLang's "CP requires intrusive per-model modifications" claim is real but training-vs-inference is genuinely asymmetric. The gap is about **which abstraction layer CP plugs into**.
>
> **Training side — CP looks generic because the attention kernel is uniform.**
>
> - Frameworks (Megatron-LM, DeepSpeed, PyTorch native CP) treat attention as a **black box**: `attn(Q, K, V, mask) → O`. Standard [[ring-attention|Ring Attention]] and [[deepspeed-ulysses|DeepSpeed Ulysses]] kernels (FlashAttention-2/-3) cover the vast majority of training models.
> - CP wraps this at the **transformer-block level**: split Q/K/V along sequence → communicate → call the standard attention kernel → merge outputs. The wrapper is written once and **every model with a standard transformer block automatically benefits**.
> - **No KV cache management** during training — a huge source of per-model complexity is simply absent.
> - Megatron *rewrites the model* end-to-end (custom transformer blocks, custom attention call sites). Per-model differences are absorbed *inside the framework's model code*, not at the CP boundary.
>
> **Inference side — CP must be per-model because attention kernels are heterogeneous.**
>
> - vLLM / SGLang do **not** rewrite models; they have to adapt to HuggingFace's wildly varying attention implementations:
>   - MHA / MQA / GQA / **MLA** (DeepSeek-V3/V4) / **NSA** (DSV4 sparse) / **sliding window** (Mistral) / **hybrid local/global** (Gemma 3) / **sink tokens** (Llama 4) / **gated delta attention** (RWKV-style) / ...
>   - Each attention variant has its own PagedAttention / FlashInfer / Triton kernel, each with a *different API*.
> - CP needs to integrate **inside or alongside** each attention kernel — it has to know the KV-cache layout, the page table format, the masking convention. None of these are standardized across models.
> - **KV cache management** adds another per-model axis. MLA's compressed KV looks nothing like GQA's standard KV. PCP / DCP have to handle each layout separately. SGLang's `--enable-nsa-prefill-context-parallel` exists *because* NSA's KV format is different enough that the generic prefill-CP code path doesn't fit.
>
> **One-sentence summary.** Training CP plugs in at the *transformer-block* level (standard interface, generically applicable); inference CP must plug in at the *attention-kernel* level (each model has a custom kernel). That's why training looks "free" once the framework supports it, but inference engines have to re-do the work for each new attention variant.
>
> **The gap is narrowing, though.** Training itself is getting more model-specific kernels — FlashAttention-3 ships NSA-style variants, MLA needs its own training kernel, DeepSeek-V4's NSA-training requires custom forward/backward. So the "training CP is generic" advantage is fading. By 2027 the training/inference asymmetry on attention-kernel diversity will probably look much smaller.
