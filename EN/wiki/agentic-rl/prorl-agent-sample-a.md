---
title: "ProRL Agent — Sample A (inverted pyramid format)"
category: agentic-rl
...
# ProRL Agent — Sample A (inverted pyramid format)

> [!warning] Format sample, not the canonical page
> This is one of two candidate redesigns of [[prorl-agent]] for the bilingual wiki's report template. The canonical page is still [[prorl-agent]] (current template). Compare with [[prorl-agent-sample-c]] (two-layer summary+depth format). See the discussion in `~/.claude/plans/lucky-tinkering-thacker.md` for context.
...
> [!abstract]+ TL;DR
> ProRL Agent makes **rollout** — running a multi-turn agent in an environment to produce a trajectory — into a standalone HTTP service that any RL trainer can consume. On SWE-Bench Verified the resulting RL training loop lifts Qwen3-4B/8B/14B Pass@1 by **6–8 percentage points** and roughly **doubles** SkyRL-Agent's reported 8B score.

---

## The one idea

> [!important] If you read only one section, read this.
> Treat agentic rollout as an HTTP service with a typed `POST /process` endpoint that produces `(token_ids, logprobs, reward)` tuples consumable by any RL trainer. Three pieces hold it up:
> 1. **Token-in / token-out wire format** — trainer and server share the canonical token sequence; no re-tokenization drift across turns.
> 2. **Three-stage async pipeline** (INIT → RUN → EVAL) with independent worker pools, so jobs at different stages don't block each other.
> 3. **Rootless HPC-compatible sandbox** (Singularity + unprivileged user + per-job loopback IP), so the system deploys on Slurm clusters where research happens.
>
...
- **Trainer swap becomes free.** veRL ↔ NeMo-RL no longer means re-porting agent loops, sandboxes, tool definitions. Anything that can `POST /process` is a valid trainer.
- **Agent and trainer iterate independently.** A new tool, a new memory module, a new scaffold change doesn't touch the RL framework. A new RLHF variant doesn't touch the agent.
- **Deployable on shared HPC.** Most agentic-RL frameworks assume Docker / root. ProRL's rootless Singularity sandbox runs on the same Slurm clusters everything else trains on — no special infra ask.
- **Concrete impact.** Doubling SkyRL-Agent's 8B SWE-Bench score isn't from algorithmic novelty — it's from the trainer no longer being I/O-starved by rollout.
...
The trainer's API contract is `① POST /add_llm_server → ② POST /start → ③ POST /process { instance, sampling_params }` (blocks) `→ ④ ← (token_ids, logprobs, reward, timing)`. The diagram below shows the structure; per-component detail follows.
...
Three components do the heavy lifting. The rest of the system is plumbing.
...
A small but practical detail: `ngram_repetition_reward(output_ids, ngram_size=64, penalty=-0.001)` applies a per-token penalty at the start position of every repeated 64-gram. This rides along with `token_ids` so the trainer can apply an off-policy-safe penalty against degenerate looping without a separate evaluation pass.
...
`OpenHandsServer.__init__` wires three queues, one per stage:
...
> - **Inter-stage time is "others" phase.** `phase_context` switches the timer between active phases; queue waiting between phases is automatically counted as `others` and excluded from the timeout budget. Without this, a worker shortage would silently fire false-negative timeouts and corrupt the training signal.

**The two-process layer outside the pipeline.** The "server" is actually a FastAPI parent process that forks a child `server_worker` process holding the `OpenHandsServer` instance. They communicate via three `multiprocessing.Queue`s — `request_queue`, `job_result_queue`, `control_response_queue`. Why two layers:
...
**Async DAPO refilling** is a systems-level extension to the published RL algorithm. DAPO filters out "Zero-Variance Prompts" (uniform reward → zero gradient). Naive batch-by-batch is synchronous waste: roll out a batch, filter, roll out another. ProRL replaces this with continuous replenishment (refill on depletion), early termination (`POST /cancel` once enough informative jobs complete), and cross-iteration persistence (unfinished jobs carry over).
...
**Tool latency: where the wall clock actually goes.** At high concurrency, tool latency dominates over LLM inference:
...
> ```
>
> A `_registries_reasoning` registry exists for reasoning-style tasks (math, STEM), so a single deployment can serve both code-agent and reasoning-agent workloads. Repository ships with handlers for SWE-Gym, R2E-Gym, SWE-Bench, math, STEM, and code.
...
> ```
>
> A whole rollout's call sequence sticks to the same backend (because `LLMConfig` is built once per `/process` and reused throughout the multi-turn loop) so prefix-cache hits stay high. Simpler than power-of-two-choices, and works because per-call cost variance is dominated by sequence length, not server-side slowdown.
...
**The cost.** Three trade-offs you accept by adopting this architecture: (1) a separate HTTP server adds an operational surface area (deployment, monitoring, version skew between trainer and server); (2) cross-process serialization adds 1–3 ms of latency per request — fine for typical multi-second rollouts, painful if your rollouts are short; (3) the rootless sandbox is Singularity-only, so deployment outside Slurm-class clusters requires either Docker (kills the rootless property) or workarounds.

**Where the paper is less convincing.** HTTP overhead is never quantified against a coupled baseline. Only DAPO is validated as the RL algorithm. The 293-instance training set is small. The `reward_server_ip` config is referenced in code but the contract is underexplained — the footnote *"No reward server IP provided. Evaluations would only work for swebench problems"* hides a load-bearing limitation.

**What it changes.** ProRL Agent is the first publicly-documented agentic-RL system that treats rollout as a first-class service, not a coroutine inside the trainer. The architectural claim — **service-oriented design beats embedded design for agentic RL, even when latency budgets are tight** — generalizes. Expect comparable services to emerge over the next 12 months for **environment-as-a-service** (cf. OpenReward in [[environment-design]]), **reward-as-a-service**, and **trajectory-store-as-a-service**. The eventual shape of an "agentic RL platform" looks more like a control plane connecting interchangeable services than a monolithic trainer.
...
- **Job-level pipeline parallelism is undervalued.** The three-stage queue gives near-linear scaling without changing any RL algorithm. Most teams leave this throughput on the table.

---

## Pointers

**Source code:** [NVIDIA-NeMo/ProRL-Agent-Server](https://github.com/NVIDIA-NeMo/ProRL-Agent-Server). Stable branch is Apache-2.0. Quick start:
...
- [[das-spec-rl]] — addresses the *other* rollout bottleneck (per-rollout decoding speed); composes cleanly with this architecture.
- [[grpo]], [[ppo-for-llm]], [[rlhf-overview]] — the RL training stack ProRL Agent serves.
...
**Paper references:** SkyRL-Agent (arXiv:2403.xxxxx), VeRL (arXiv:2402.xxxxx), Agent Lightning (Microsoft 2024), rLLM (Stanford 2025), GEM (Google DeepMind 2025).
