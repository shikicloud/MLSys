---
title: "ProRL Agent — Sample C (two-layer summary + depth format)"
category: agentic-rl
...
# ProRL Agent — Sample C (two-layer summary + depth format)

> [!warning] Format sample, not the canonical page
> This is one of two candidate redesigns of [[prorl-agent]] for the bilingual wiki's report template. The canonical page is still [[prorl-agent]] (current template). Compare with [[prorl-agent-sample-a]] (inverted-pyramid format). See the discussion in `~/.claude/plans/lucky-tinkering-thacker.md` for context.
...
**Why it matters.** Trainer swap (veRL ↔ NeMo-RL) stops requiring agent-loop and sandbox re-porting; agent and trainer can iterate independently; and the whole thing deploys on the same shared HPC clusters where training already lives — no Docker-daemon dependency. By 2027 expect this "rollout-as-a-service" pattern to be the default agentic-RL architecture, with related services emerging for environment-as-a-service, reward-as-a-service, and trajectory-store-as-a-service.
...
>
> Three terms in this Background section warrant precise definitions, since the paper's claims rest on them.
>
> **Scaffold** — In agentic-RL terminology, the *scaffold* is everything between the user's task and the LLM's `model.generate()` call: the agent loop (ReAct, plan-and-execute), tool definitions, prompt templates, memory management, parsing of tool calls, retry logic. ProRL's "scaffold-independent" column means the rollout server's HTTP API doesn't bake in any specific scaffold — plug in OpenHands's CodeAct, an in-house ReAct loop, or anything else, as long as it implements `AgentHandler`.
>
> **Stable HTTP contract** — A versioned, well-defined HTTP API where the request/response schemas are fixed (here via Pydantic models). Stability means: (1) the schema doesn't break in incompatible ways without version negotiation; (2) different versions of trainer and server can interop. It's the only coupling between the two processes — at any moment one team can rewrite the server, switch its language, or move it across machines, and the trainer doesn't care, as long as `POST /process` still accepts a `ProcessRequest` and returns `(token_ids, logprobs, reward)`.
>
> **Rootless sandbox** — A container/sandbox that runs entirely as an unprivileged user — no root anywhere in the chain. Docker is famously *not* rootless: it requires a daemon running as root. On HPC clusters managed by Slurm you don't have root and there's no Docker daemon, so Docker simply can't run. **Singularity** (now **Apptainer**) was designed for this: containers execute in user space using user namespaces, and `--fakeroot` gives the *appearance* of root *inside* the container without escalating privileges outside. ProRL Agent uses `.sif` images so it deploys on the same shared HPC clusters the rest of training infrastructure already lives on.
...
There's also a corner case: when a Qwen3 model emits `<think>\n` followed immediately by tool calls but never closes with `</think>`, the HuggingFace tokenizer breaks. The agent post-processes this by compressing the tool-call content back into the `content` string and appending a synthetic `</think>` so the canonical token IDs stay parseable.
...
> - **Inter-stage time is "others" phase.** `phase_context` switches the timer between active phases; queue waiting between phases is automatically counted as `others` and excluded from the timeout budget. Without this, a worker shortage during a load spike would silently fire false-negative timeouts.
...
def process(self, instance, sampling_params, job_id=None, timeout=300.0):
    ...
    job_details = JobDetails(job_id=job_id or self.get_unique_id(instance),
...
**Async DAPO refilling.** [[grpo|DAPO]] filters out "Zero-Variance Prompts" (uniform reward → zero gradient). Naive batch-by-batch is synchronous waste. ProRL replaces this with three primitives: continuous replenishment (refill on depletion), early termination (`POST /cancel` once enough informative jobs complete), and cross-iteration persistence (unfinished jobs carry over).
...
**AgentHandler plugin interface.** Task-specific logic plugs in via an abstract base class (`openhands/nvidia/registry.py`):
...
class AgentHandler(ABC):
    ...
    @abstractmethod
    async def init(self, job_details, sid=None) -> tuple[Runtime, EvalMetadata, OpenHandsConfig]: ...
    @abstractmethod
    async def run(self, job_details, sid=None) -> dict[str, object]: ...
    @abstractmethod
    async def eval(self, job_details, sid=None, allow_skip=True, reward=None) -> dict[str, Any]: ...
    @abstractmethod
    def final_result(self, job_details) -> dict[str, Any]: ...
```
...
**LLM backend load balancing.** The min-heap is two lines plus the lock:
...
A whole rollout's call sequence sticks to the same backend (because `LLMConfig` is built once per `/process` and reused) so prefix-cache hits stay high. Simpler than power-of-two-choices, and works because per-call cost variance is dominated by sequence length, not server-side slowdown.

## Experimental evidence

**Setup.** 32× NVIDIA H100. RL algorithm: DAPO ([[grpo|GRPO]] variant with Zero-Variance-Prompt filtering). Batch 32, mini-batch 8, 8 rollouts per instance, KL = $10^{-4}$, lr = $10^{-6}$. Models: Qwen3 family.

**Main result on SWE-Bench Verified** (293-instance SWE-Gym training subset):
...
The 8B number is roughly **2× SkyRL-Agent-8B-v0's 9.4 %**.
